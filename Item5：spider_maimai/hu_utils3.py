...
def open_local_db(db="spider"):
    ...
    conn = pymysql.connect(host='192.168.1.4', user='root', passwd='666', db=db, port=3306, charset='gbk')
    return conn
...
def open_line_db(db="lz_datastore"):
    ...
    conn = pymysql.connect(host='rm-2zeuz492sf50a2t27ko.mysql.rds.aliyuncs.com',
                           user='gcdata2012',
                           passwd='gdlz_2017',
                           db=db,
                           port=3306,
...
def select_one(conn):
    ...
    """
    cursor = conn.cursor()
    sql = "select ios_id from ios_main"
    try:
        cursor.execute(sql)
...
def insert_update_one(conn, item, table_name,status=True):
    """
...
def insert_update_many(conn, items, table_name,status=True):
    """
