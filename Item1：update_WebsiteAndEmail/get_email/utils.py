...
def open_line_db(db="lz_datastore"):
    """
    开启本地数据库
    :param db: 数据库（默认为lz_datastore）
    :return: 创建好的数据库连接
    """
    print time.ctime(), '连接线上数据库%s' % db
...
def query(ls):
    print current_time(), '线上查询是否重复...'
    conn = open_line_db()
    cursor = conn.cursor()
    cursor.executemany("select etid,ecemail from et_contact where etid=%s and ecemail=%s", ls)
...
def insert_update_con(conn, items, table_name):
    ...
    :return:无
    """
    logging.info('%s 开始将数据更新到数据表et_contact ' % current_time())
    cursor = conn.cursor()
    sql1 = "insert into %s" % table_name
    sql2 = "(etid, ecemail) values(%s,%s)"
    sql = sql1 + sql2
    num = len(items)
    print time.ctime(), '一共需要处理数据%s条' % num
    try:
